...
if selected_mode == "📝 New Content":
    ...
    if st.session_state.current_step == 1:
            ...
            if st.session_state.optimization_data.get('analysis_complete'):
                ...
                if analysis_result.get('success'):
                    ...
                    tab1, tab2, tab3, tab4 = st.tabs([
                        f"✅ Keep ({len(keep_recs)})",
                        f"🔧 Improve ({len(improve_recs)})",
                        f"➕ Add ({len(add_recs)})",
                        f"❌ Remove ({len(remove_recs)})"
                    ])
                    ...
                    with tab1:
                        if keep_recs:
                            ...
                            st.info("No sections to keep as-is")
                    ...
                    with tab2:
                        if improve_recs:
                            ...
                            st.info("No sections need improvement")
                            ...
                            for rec in add_recs:
                                with st.container():
                                    ...
                                    if rec.get('h3_subheadings'):
                                        st.markdown("**Suggested subheadings:**")
                                        for h3 in rec['h3_subheadings']:
                                            st.markdown(f"  • {h3}")
                                    st.markdown("")
                        else:
                            st.info("No new sections to add")
        ...
        if not st.session_state.optimization_data.get('optimized_headings'):
            st.warning("Please complete Structure Editor first")
        ...
        if st.button("🚀 Generate Optimized Content", type="primary"):
            with st.spinner("Generating & humanizing optimized content..."):
                progress_bar = st.progress(0)
                status_text = st.empty()
